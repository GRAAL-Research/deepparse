...
class EmbeddingNetwork(nn.Module):
    def __init__(self, input_size, hidden_size, projection_size, batch_size, device, num_layers=1, maxpool=False,
    ...
    def forward(self, input_, decomposition_lengths):
        ...
        for i in range(input_.size(0)):
            ...
            packed_sequence = nn.utils.rnn.pack_padded_sequence(input_[i], torch.tensor(lengths).to(self.device),
                                                                batch_first=True, enforce_sorted=False)

            packed_output, hidden = self.model(packed_sequence, self.hidden)

            padded_output, padded_output_lengths = nn.utils.rnn.pad_packed_sequence(packed_output, batch_first=True,
                                                                                    padding_value=-100)
