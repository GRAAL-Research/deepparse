...
class AccuracyTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.a_prediction_tensor = torch.tensor([[[
            ...
            -1.9178e+01
                                                ...
                                                requires_grad=True)
        self.an_accuracy_score = torch.tensor(50., device=self.a_device)
    ...
    def test_givenAPerfectPredictionTensor_whenAccuracyPerTag_thenAccuracyIs100(self):
        first_token_first_element_of_the_batch = [1, 0]  # the predicted token is the first class
        first_token_second_element_of_the_batch = [0, 1]  # the predicted token is the second class
        second_token_first_element_of_the_batch = [0, 1]  # the predicted token is the second class
        second_token_second_element_of_the_batch = [1, 0]  # the predicted token is the first class
        ...
        expected = torch.tensor(100., device=self.a_device)
        self.assertEqual(expected, actual)

    def test_givenACompletelyWrongPredictionTensor_whenAccuracyPerTag_thenAccuracyIs0(self):
        first_token_first_element_of_the_batch = [0, 1]  # the predicted token is the second class
        first_token_second_element_of_the_batch = [1, 0]  # the predicted token is the first class
        second_token_first_element_of_the_batch = [1, 0]  # the predicted token is the first class
        second_token_second_element_of_the_batch = [0, 1]  # the predicted token is the second class
        predict_tensor = torch.tensor(
            [[first_token_first_element_of_the_batch, first_token_second_element_of_the_batch],
             [second_token_first_element_of_the_batch, second_token_second_element_of_the_batch]],
            device=self.a_device)
